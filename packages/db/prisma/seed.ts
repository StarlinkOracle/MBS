...
const permissions = [
  ...
  { key: 'hvac:diagnostic:read', description: 'Read HVAC diagnostic sessions, evidence, findings, and reports' },
  { key: 'hvac:diagnostic:write', description: 'Create and update HVAC diagnostic sessions and measurements' },
  { key: 'hvac:diagnostic:finalize', description: 'Technician confirmation of HVAC diagnostic outcomes and reports' },
  { key: 'hvac:oem:read', description: 'Read reviewed HVAC manufacturer references' },
  { key: 'hvac:oem:manage', description: 'Create and review HVAC manufacturer document references' },
];

const roleDefinitions: Record<string, string[]> = {
  admin: ['*'],
  dispatcher: ['hvac:diagnostic:read', 'crm:read', 'crm:write', 'crm:lead:read', 'crm:quote:read', 'jobs:*', 'scheduling:read', 'scheduling:write', 'media:read', 'media:write', 'media:share', 'comms:read', 'comms:write', 'comms:triage', 'time:read', 'time:write', 'time:edit:review', 'mobile:sync', 'auth:mobile:pin:set', 'auth:mobile:pin:reset', 'system:ops:read', 'reporting:*'],
  billing_manager: [
    'crm:read',
    'crm:write',
...
const toolCatalog: ToolSeed[] = [
  {
    name: 'hvac.diagnostic.session.create',
    description: 'Create a CRM-linked HVAC diagnostic session and equipment record.',
    handlerKey: 'hvac.diagnostic.session.create',
    riskLevel: RiskLevel.LOW,
    autonomyLevel: AutonomyLevel.ALWAYS_ALLOWED,
    requiredPermissions: ['hvac:diagnostic:write'],
    inputSchema: {
      type: 'object',
      ...
      properties: {
        ...
        equipmentSystemId: { type: 'string' },
        equipmentSystem: { type: 'object', additionalProperties: true },
  ...
  {
    name: 'hvac.diagnostic.session.update',
    description: 'Update the structured state or linked equipment for an HVAC diagnostic session.',
    handlerKey: 'hvac.diagnostic.session.update',
    riskLevel: RiskLevel.LOW,
    autonomyLevel: AutonomyLevel.ALWAYS_ALLOWED,
    requiredPermissions: ['hvac:diagnostic:write'],
    inputSchema: {
      ...
      properties: {
        ...
        operatingMode: { type: ['string', 'null'] },
        altitudeFt: { type: ['number', 'null'] },
        equipmentSystemId: { type: ['string', 'null'] },
        equipmentSystem: { type: 'object', additionalProperties: true },
        equipmentComponents: { type: 'array', items: { type: 'object', additionalProperties: true } },
        state: { type: 'object', additionalProperties: true },
        status: { type: 'string' },
      },
      required: ['sessionId'],
  ...
  {
    name: 'hvac.diagnostic.measurement.record',
    description: 'Record raw HVAC field measurements and run deterministic calculations and evidence rules.',
    handlerKey: 'hvac.diagnostic.measurement.record',
    riskLevel: RiskLevel.LOW,
    autonomyLevel: AutonomyLevel.ALWAYS_ALLOWED,
    requiredPermissions: ['hvac:diagnostic:write'],
    inputSchema: {
      ...
      required: ['sessionId', 'measurements'],
    },
  },
  {
    name: 'hvac.diagnostic.message.add',
    description: 'Append a governed technician or assistant message to an HVAC diagnostic conversation.',
    handlerKey: 'hvac.diagnostic.message.add',
    riskLevel: RiskLevel.LOW,
    autonomyLevel: AutonomyLevel.ALWAYS_ALLOWED,
    requiredPermissions: ['hvac:diagnostic:write'],
    inputSchema: {
      ...
      properties: {
        sessionId: { type: 'string' },
        role: { type: 'string', enum: ['SYSTEM', 'ASSISTANT', 'TECHNICIAN'] },
        text: { type: 'string' },
  ...
  {
    name: 'hvac.diagnostic.verification.record',
    description: 'Record the test used to verify or reject an HVAC diagnostic finding.',
    handlerKey: 'hvac.diagnostic.verification.record',
    riskLevel: RiskLevel.LOW,
    autonomyLevel: AutonomyLevel.ALWAYS_ALLOWED,
    requiredPermissions: ['hvac:diagnostic:write'],
    inputSchema: {
      ...
      required: ['sessionId', 'title', 'procedure'],
    },
  },
  {
    name: 'hvac.diagnostic.finalize',
    description: 'Require human technician confirmation, lock the outcome, and generate technical and customer reports.',
    handlerKey: 'hvac.diagnostic.finalize',
    riskLevel: RiskLevel.MEDIUM,
    autonomyLevel: AutonomyLevel.NEVER_AUTONOMOUS,
    requiredPermissions: ['hvac:diagnostic:finalize'],
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
        ...
        finalDiagnosisEvidence: { type: 'object', additionalProperties: true },
        technicianConfirmed: { const: true },
      },
      required: ['sessionId', 'outcome', 'technicianConfirmed'],
    },
    requiresReason: true,
    requiresSnapshot: true,
  },
  {
    name: 'hvac.oem.reference.upsert',
    description: 'Create or update a human-reviewed structured reference from an HVAC manufacturer document.',
    handlerKey: 'hvac.oem.reference.upsert',
    riskLevel: RiskLevel.HIGH,
    autonomyLevel: AutonomyLevel.NEVER_AUTONOMOUS,
    requiredPermissions: ['hvac:oem:manage'],
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
  ...
  {
    name: 'hvac.oem.reference.lookup',
    description: 'Look up reviewed OEM targets by manufacturer, model, operating mode, and refrigerant.',
    handlerKey: 'hvac.oem.reference.lookup',
    riskLevel: RiskLevel.LOW,
    autonomyLevel: AutonomyLevel.ALWAYS_ALLOWED,
    requiredPermissions: ['hvac:oem:read'],
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
        ...
        model: { type: 'string' },
        equipmentComponentType: { type: 'string' },
        operatingMode: { type: 'string' },
